...
def view_graph(db, output_dir, imgs_dir):
    print("Creating viewing graph visualization")
    ...
    for pair_id, rows in res.fetchall():
        if rows != 0:
            ...
            img2 = int(img2)
            if img1 not in G:
                res_images = cur.execute("SELECT name from images WHERE image_id = ?", [img1])
                name = res_images.fetchone()[0]
                G.add_node(img1, label = name, shape = "ellipse")
            if img2 not in G:
                res_images = cur.execute("SELECT name from images WHERE image_id = ?", [img2])
                name = res_images.fetchone()[0]
    ...
    if (G.number_of_nodes() <= 30):
        for n in G.nodes():
            ...
            print(os.path.abspath(os.path.join(imgs_dir, G.nodes[n]["label"])))
            G.nodes[n]["image"] = os.path.abspath(os.path.join(imgs_dir, G.nodes[n]["label"]))
