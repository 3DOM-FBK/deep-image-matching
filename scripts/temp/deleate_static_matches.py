...
matches_file = rf"{WORK_DIR}\matches\matches.txt"
kpts_dir = rf"{WORK_DIR}\colmap_desc"
out_dir = rf"{WORK_DIR}\no_static"


def read_matches_txt(path_to_matches, matches_dict):
    with open(path_to_matches) as matches_file:
...
for key in matches_dict:
    ...
    with (
        open(f"{kpts_dir}/{img1}.txt") as kp1_file,
        open(f"{kpts_dir}/{img2}.txt") as kp2_file,
        open(f"{out_dir}\matches_no_static.txt", "a") as out_matches_file,
    ):
        ...
        # print(c1, c2)
        out_matches_file.write(f"{img1} {img2}\n")
        ...
        for item in matches_dict[key]:
            ...
            if (
                ...
                # print('ok')
                out_matches_file.write(f"{m1} {m2}\n")
