...
def featureMatching():
  ...
  for image1_index, image2_index in tqdm((np.loadtxt(args.pair_list, dtype=np.int32) if args.pair_list != None else np.asarray([*combinations(view_ids, 2)], dtype=np.int32))):
    ...
    lafs1 = KF.laf_from_center_scale_ori(keyp1[None], 96 * torch.ones(1, len(keyp1), 1, 1, device=device))
    lafs2 = KF.laf_from_center_scale_ori(keyp2[None], 96 * torch.ones(1, len(keyp2), 1, 1, device=device))
...
if __name__ == '__main__':
  ...
  device = torch.device('cuda') #if args.force_cpu else K.utils.get_cuda_device_if_available()
